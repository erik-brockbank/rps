...
connectToServer = function() {
    ...
    game.socket.on("onconnected", client_onconnected); // message noting that we're connected
    game.socket.on("newgame", client_waitingroom_enter.bind(game)); // message that we're the first to join a new game
    game.socket.on("roundbegin", client_begin_round.bind(game)); // message that we're starting a new round (also joining an existing game)
    game.socket.on("roundwaiting", client_waiting); // message that we're waiting for the opponent at any point during the game
    game.socket.on("roundcomplete", client_display_results.bind(game)); // message that we've completed the current round
    game.socket.on("gameover", client_finish_game); // message that we've completed the game
};
...
client_waitingroom_enter = function(data) {
    ...
    var wait_msg = "Waiting for an opponent to join...";
    clientDisplayMessage(wait_msg, hideall=true);
};
...
client_begin_round = function(data) {
    ...
    start_time_ms = new Date().getTime(); // more accurate timer for participant responses TODO try replacing the above with this
...
client_display_results = function(data) {
    ...
    console.log("client.js:\t displaying results of round: ", data);
    if (player.client_id == data.player1.client_id) {
