...
finish_game = function() {
    ...
    // TODO move this to client_view
    $("#game-banner").hide();
    $("#client-info").hide();
    $("#opponent-info").hide();
    $("#game-info").hide();
    show_next_button(exit_survey.bind(this));
};


exit_survey = function() {
    console.log("client.js:\t beginning exit survey.");
    survey_administer_free_resp(survey_administer_sliders, this);
};
...
survey_administer_free_resp = function(callback, game) {
    ...
    $("#experiment-container").load(HTML_LOOKUP["survey_free_resp"], function() {
        hide_next_button();
        $("#survey-banner").text(SURVEY_FREE_RESPONSE);
        $("#exp-button-container").css({visibility: "visible"});
        $("#next-free-resp").unbind().click(function() {
...
survey_administer_sliders = function(game) {
    ...
    $("#experiment-container").load(HTML_LOOKUP["survey_slider"], function() {
        hide_next_button();
        $("#survey-banner").text(SURVEY_SLIDER_MESSAGE);
        survey_slider_questions(game, slider_index = 0, slider_data = [], callback = end_survey);
    });
};

survey_slider_questions = function(game, slider_index, slider_data, callback) {
    $("#survey-eval").text(SURVEY_ARRAY[slider_index]);
    $("#exp-button-container").css({visibility: "visible"});
    $("#next-slider").unbind().click(function() {
        slider_data.push({
            game_id: THIS_PLAYER.game_id,
            player_id: THIS_PLAYER.client_id,
...
end_survey = function() {
    ...
    $("#experiment-container").html("<h2>Thank you for playing!</h2>");
};
