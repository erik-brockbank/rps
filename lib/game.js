...
rps_game = function(game_id = null, version = "1", istest = false, player1 = null, player2 = null, game_rounds = null) {
...
// NB: this has the same fields as rps_player but some additional functions defined for selecting moves etc.
rps_bot_player = function() {
    this.client_id = "bot";
    this.status = null;
    this.fixed_strategy = null;
};


// Function to choose a move based on the fixed strategy
// TODO fill this in...
rps_bot_player.prototype.chooseMove = function() {
    if (this.fixed_strategy == "") {
        return "rock"
    } else {
        return "scissors"
    }
...
// TODO move bot functions to a separate js file?
// TODO possible race between bot and participant move choice if participant is really fast?
rps_bot_player.prototype.makeMove = function(game) {
    ...
    if (current_round.player1_move && current_round.player2_move) {
        ...
        game.current_round = current_round;
        game.player1_points_total += current_round.player1_points; // update game total points
        game.player2_points_total += current_round.player2_points; // update game total points
...
rps_game_server.prototype.findGame = function(client, version, istest) {
    ...
    if (version == "1" && Object.keys(this.active_games).length > 0) {
        for (game_id in this.active_games) {
            ...
            if (!game.player1 || !game.player2) {
                ...
                // TODO try passing in game_id instead and in the function update this.active_games[game_id] = modified game
    ...
    this.createGame(client, version, istest);
};


// Function to create a new game and add this client as the first player
rps_game_server.prototype.createGame = function(client, version, istest) {
    ...
    newgame_id = UUID();
    var newgame = new rps_game(game_id = newgame_id, version = version, istest = istest,
    ...
    if (version == "2") {
        bot_opponent = this.createBotPlayer(version);
    ...
    if (version == "2") {
        // TODO instead of passing in game (by value), call this function and have it fetch the game and update it
        this.beginBotGame(newgame);
    }
};


// Function to generate a bot player with a fixed strategy
rps_game_server.prototype.createBotPlayer = function(version) {
    var bot_op = new rps_bot_player();
    bot_op.fixed_strategy = "";
    console.log("game.js:\t creating new bot player: ", bot_op);
    return bot_op
};


// Function to add client to an existing game that needs an opponent
// TODO have this function fetch the game and update it rather than passing it in
rps_game_server.prototype.addPlayerToGame = function(game, client, istest) {
    console.log("game.js:\t adding client to existing game");
    // Create new player for this client
    var newplayer = new rps_player(client.userid);
...
// TODO have this function fetch the game and update it rather than being passed in
rps_game_server.prototype.beginBotGame = function(game) {
    // Modify relevant fields in existing game
    game.current_round_index = 1;
    game.game_begin_ts = new Date().getTime(); // unix timestamp
    // Create new round and add to game
    ...
    console.log("game.js:\t initializing bot game: ", game);
    game.player1_client.emit("roundbegin", this.copyGameVals(game));
    var bot = game.player2;
    bot.makeMove(game);
};
...
rps_game_server.prototype.processMove = function(client, data) {
    ...
    if (current_round.player1_move && current_round.player2_move) {
        ...
        // Update both clients (or single client if playing against a bot)
        game.player1_client.emit("roundcomplete", this.copyGameVals(current_game));
        if (game.player2.client_id != "bot") {
            game.player2_client.emit("roundcomplete", this.copyGameVals(current_game));
        } else {
            game.player2.status = "waiting_for_opponent";
        }
    }
};
...
rps_game_server.prototype.writeFreeRespData = function(data) {
...
rps_game_server.prototype.writeSliderData = function(data) {
