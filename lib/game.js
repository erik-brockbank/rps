...
AdaptiveBotPlayer = function() {
    ...
    this.memory_struct = ADAPATIVE_BOT_MEMORY_LOOKUP[this.adaptive_strategy]; // get "memory" structure for this bot's adaptive strategy
};
...
AdaptiveBotPlayer.prototype.chooseMove = function(game) {
    ...
    if (game.current_round_index <= 1 ||
        ...
        if (this.adaptive_strategy == "opponent_transitions") {
            // choose move that would win against most probably human opponent move, given human transition proportions (relative to own move)
            var prev_round = game.previous_rounds[game.current_round_index - 2];
            if (prev_round.player1_move != "none") { // TODO consider replacing "in memory_struct" with this
        ...
        if (this.adaptive_strategy == "opponent_courn_transitions") {
            ...
            var prev_round = game.previous_rounds[game.current_round_index - 2];
            if (prev_round.player2_move != "none") { // TODO consider replacing "in memory_struct" with this
        ...
        if (this.adaptive_strategy == "opponent_outcome_transitions") {
            ...
            var prev_round = game.previous_rounds[game.current_round_index - 2];
            if (prev_round.player1_move != "none") {
        ...
        if (this.adaptive_strategy == "opponent_outcome_prev_transition_dual") {
            var prev_round = game.previous_rounds[game.current_round_index - 2];
            var ante_prev_round = game.previous_rounds[game.current_round_index - 3];
            if (prev_round.player1_move != "none" && ante_prev_round.player1_move != "none") {
                ...
                var move = WINNING_MOVE_LOOKUP[TRANSITION_MOVE_MAP[next_transition][prev_round.player1_move]];
                return move;
            } else {
                return weighted.select(BOT_RANDOM_MOVES);
            }
...
AdaptiveBotPlayer.prototype.updateMemory = function(game) {
    ...
    if (this.adaptive_strategy == "opponent_transitions") {
        ...
        // TODO should the above be replaced by a similar check here?
        if (prev_round && prev_round.player1_move != "none" && current_round.player1_move != "none") {
    ...
    if (this.adaptive_strategy == "opponent_courn_transitions") {
        ...
        if (prev_round && prev_round.player2_move != "none" && current_round.player1_move != "none") {
...
GameServer.prototype.processMove = function(client, data) {
    ...
    if (current_round.player1_move && current_round.player2_move) {
        ...
        if (current_game.player2.client_id == "adaptive_bot") {
            ...
            current_game.current_round.player2_memory_struct = JSON.parse(JSON.stringify(current_game.player2.memory_struct));
        }
...
GameServer.prototype.writeData = function(current_game) {
    ...
    if (current_game.game_id && current_game.player1 && current_game.player2) {
        ...
        if (current_game.version == "3" && current_game.player2.client_id == "adaptive_bot") {
            ...
            data_obj["player2_opponent_memory"] = JSON.parse(JSON.stringify(current_game.player2.memory_struct));
        }
        ...
        for (round_idx in current_game.previous_rounds) {
            ...
            if (current_game.version == "3" && current_game.player2.client_id == "adaptive_bot") {
                round_obj.player2_memory_struct = JSON.parse(JSON.stringify(round.player2_memory_struct));
            }
