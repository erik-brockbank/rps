...
AdaptiveBotPlayer.prototype.chooseMove = function(game) {
    ...
    if (game.current_round_index == 1) { // TODO scale this out for other strategies (round 2, 3, etc. in adaptive transition strategies)
        // sample randomly from "rock", "paper", "scissors"
        return weighted.select(BOT_RANDOM_MOVES);
    } else {
        if (this.adaptive_strategy == "opponent_prev_moves") {
            ...
            var opponent_moves = this.memory_struct;
            return WINNING_MOVE_LOOKUP[_.sample(maxMove(opponent_moves))];
        }
    }
};
...
AdaptiveBotPlayer.prototype.updateMemory = function(game) {
    ...
    var current_round = game.current_round;
    if (this.adaptive_strategy == "opponent_prev_moves") {
        // Only update if human player actuall chose a move...
