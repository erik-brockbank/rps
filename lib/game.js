 ...
 */
Game = function(game_id = null, version = "1", istest = false, player1 = null, player2 = null, game_rounds = null) {
...
BotPlayer.prototype.chooseMove = function(game) {
    if (game.current_round_index == 1) {
        ...
        // If bot is using its own previous move to determine next move, fetch its previous move
        if (this.fixed_strategy == "prev_move_positive" || this.fixed_strategy == "prev_move_negative") {
        ...
        } else if (this.fixed_strategy == "opponent_prev_move_positive" || this.fixed_strategy == "opponent_prev_move_nil"){
            var prev_move = prev_round.player1_move;
        }
        if (prev_move == "none") {
            // If previous move was none, we have nothing to use.
            // sample randomly from "rock", "paper", "scissors"
            return weighted.select(BOT_RANDOM_MOVES);
        } else {
            // Select move based on probabilities associated with transition from whichever previous move is relevant to this strategy
            return weighted.select(this.fixed_strategy_transitions[prev_move]);
        }
    }
}
...
GameServer.prototype.endGame = function(game_id, rps_player) {
    current_game = this.active_games[game_id];
    if (current_game && rps_player && rps_player.client_id != "bot") {
