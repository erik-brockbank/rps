...
AdaptiveBotPlayer.prototype.chooseMove = function(game) {
    ...
    if (game.current_round_index <= 1 ||
        (game.current_round_index <= 2 && _.includes(["opponent_prev_move", "bot_prev_move", "opponent_bot_prev_move"], this.adaptive_strategy)) ||
        (game.current_round_index <= 3 && _.includes(["opponent_prev_two_moves", "bot_prev_two_moves"], this.adaptive_strategy))) {
        console.log("HAZARD RANDOM");
        // sample randomly from "rock", "paper", "scissors"
        return weighted.select(BOT_RANDOM_MOVES);
    } else {
        ...
        if (this.adaptive_strategy == "bot_prev_two_moves") {
            ...
            if (prev_round.player2_move in memory_struct && ante_prev_round.player2_move in memory_struct) {
                // var move = WINNING_MOVE_LOOKUP[_.sample(maxMove(memory_struct[ante_prev_round.player2_move][prev_round.player2_move]))];
                // console.log("memory: ", memory_struct);
                // console.log("bot move 2back: ", ante_prev_round.player2_move);
                // console.log("bot move 1back: ", prev_round.player2_move);
...
AdaptiveBotPlayer.prototype.updateMemory = function(game) {
    ...
    if (this.adaptive_strategy == "bot_prev_two_moves") {
        ...
        if (ante_prev_round && prev_round &&
            ...
            current_round.player1_move in this.memory_struct) {
            // console.log("memory: ", this.memory_struct);
            // console.log("bot move 2-back: ", ante_prev_round.player2_move);
            // console.log("bot previous move: ", prev_round.player2_move);
            // console.log("current player move: ", current_round.player1_move);
    ...
    // console.log("updated memory struct: ", this.memory_struct);
...
GameServer.prototype.writeData = function(current_game) {
    ...
    if (current_game.game_id && current_game.player1 && current_game.player2) {
        ...
        if (current_game.version == "3" && current_game.player2.client_id == "adaptive_bot") {
            ...
            data_obj["player2_opponent_memory"] = current_game.player2.memory_struct;
        }
