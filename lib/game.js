...
rps_game_server.prototype.processMove = function(client, data) {
    ...
    if (!data.hasOwnProperty("move") || (rps_player_move.indexOf(data.move) == -1)) {
        console.error("ERROR game.js:\t Invalid move in data: ", data);
    }
...
rps_game_server.prototype.nextRound = function(client) {
    console.log("next round request from client: ", client.userid);
    ...
    if (client.userid == current_game.player1.client_id) {
        if (current_game.current_round_index == GAME_ROUNDS) {
            current_game.player1.status = "exited"; // end of game, client should see game over
        } else {
            current_game.player1.status = "waiting_for_opponent"; // still in game
        }
    } else if (client.userid == current_game.player2.client_id) {
        if (current_game.current_round_index == GAME_ROUNDS) {
            current_game.player2.status = "exited"; // end of game, client should see game over
        } else {
            current_game.player2.status = "waiting_for_opponent"; // still in game
        }
    }
    ...
    // If this wasn't the final round of the game, proceed to next round as usual
        ...
        if (current_game.player1.status != "waiting_for_opponent") {
            current_game.player2_client.emit("roundwaiting_continue");
            return;
        } else if (current_game.player2.status != "waiting_for_opponent") {
            current_game.player1_client.emit("roundwaiting_continue");
            return;
        // If both clients are ready, update them by starting the next round
            ...
            current_game.game_status = "in_play";
            // Update both clients
...
rps_game_server.prototype.clientDisconnect = function(client) {
    ...
    // Identify game and round information for this client
    current_game = this.getCurrentGame(client);
    ...
    if (current_game) {
        ...
        if (client.userid == current_game.player1.client_id) {
            ...
            if (current_game.player1.status != "exited") {
                ...
                // Write data even if we didn't get a full set
                // NB: this will not double write if the other client exits
                this.writeData(this.copyGameVals(current_game));
                // Remove game from game_server
                delete this.active_games[current_game.game_id];
            }
        } else if (client.userid == current_game.player2.client_id) {
...
rps_game_server.prototype.endGame = function(client) {
    if (client) {
        console.log("game.js:\t sending gameover for client: ", client.userid);
...
2. clean up objects, remove unnecessary items (game.current_round.player1 has a game field that's null, etc.)
